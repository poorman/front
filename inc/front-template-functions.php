...
if ( ! function_exists( 'front_single_post_classic_header' ) ) :
    ...
    function front_single_post_classic_header() {
        ...
        if( ! empty( $cover_image_id ) ) {
            $cover_image = get_the_post_thumbnail_url( null, 'full' );
           // $cover_image = wp_get_attachment_url( $cover_image_id );
        } else {
            $cover_image = get_the_post_thumbnail_url( null, 'full' );
        }

        ?><div class="dzsparallaxer <?php if ( ! empty( $cover_image ) ): ?>auto-init <?php endif; ?>height-is-based-on-content use-loading mode-scroll" data-options='{direction: "normal"}'>
    ...
    function front_single_post_simple_thumbnail() {
        ...
        <div class="col-lg-6 position-lg-absolute top-lg-0 right-lg-0 px-lg-0">
            ...
            data-options='{direction: "normal"}'>
                <!-- Apply your Parallax background image here -->
                <div class="divimage dzsparallaxer--target" style="height: 130%;<?php if( ! empty( $cover_image ) ) { echo ' background-image: url(' . esc_url( $cover_image ) . ');'; } ?>"></div>
            </div>
        </div>
...
if ( ! function_exists( 'front_single_post_classic_content' ) ) {
    function front_single_post_classic_content() {
        $has_sidebar = function_exists( 'front_single_post_has_sidebar' ) ? front_single_post_has_sidebar() : false;
        $layout = function_exists( 'front_get_single_post_layout' ) ? front_get_single_post_layout() : 'full-width';
        ?>
        <div class="article__content article__content--post container space-top-2 space-bottom-2">
            <?php if( $has_sidebar ) :
                $content_classes = 'single-post-content col-lg-9 mb-9 mb-lg-0';

                if ( $layout === 'sidebar-left' ) {
                    $content_classes .= ' order-lg-2';
                }
                ?>
                <div class="row"><div class="<?php echo esc_attr( $content_classes ); ?>">
            <?php else : ?>
            ...
            <?php endif; ?>
                    ...
                    <div class="article-details">
                        ...
                        <span class="post-author">
                                   <?PHP    $alt = get_the_author_meta( 'display_name' ).' mortgage expert';
                                 echo get_avatar( get_the_author_meta('user_email'), $size = '70', $default='', $alt= $alt); ?>
                                <span class="author-style">
                                    <a class="link" href="#"><?php echo get_the_author(); ?></a> The Mortgage Expert
                                </span>
                               <br>
                                <span class="post-date">
                                    <i class="fa fa-calendar"></i> <?php echo esc_html(get_the_date()); ?>
                                    <span class="reading_time">- <?php echo do_shortcode('[rt_reading_time postfix="min read" postfix_singular="min read"] '); ?></span>
                                </span>
                            </span>
                          </div>
                        </div>
             <?php
                ...
                ?>
            <?php if( $has_sidebar ) : ?>
                </div><?php get_sidebar(); ?></div><?php front_sticky_block_endpoint(); ?>
            <?php else : ?>
                </div>
            <?php endif; ?>
        </div>
...
function front_single_post_simple_footer() {
    ...
    if( ! empty( $output_content ) ) {
        echo sprintf( '<div class="container space-bottom-2"><div class="w-lg-75 mx-auto">%s</div></div>', $output_content );
    }
}
endif;
